...
public class CourtFragment extends BaseFragment implements LoadListener, OnHolderListener {
  ...
  private void setView(Bundle bundle) {
    ...
    mCourtMarksCount.setText(String.valueOf(bundle.getInt(EXTRA_COURT_MARKS_COUNT)));
        ...
        .subscribe(
            response -> {
              if (response.getTotalEntries() != null) {
                mFoundCount.setText(
                    String.valueOf(response.getTotalEntries())
                        + StringUtils.SPACE
                        + StringUtils.JUDGES_WERE_FOUND);
              } else {
                mFoundCount.setText(
                    StringUtils.ZERO + StringUtils.SPACE + StringUtils.JUDGES_WERE_FOUND);
              }
              if (page == 1) {
                mJudgeList.clear();
